...
class PySelenium(object):
    ...
    def __init__(self, browser='ff', remoteAddress=None):
        ...
        try:
            self.driver = dr
            self.my_print("{0} Start a new browser: {1}, Spend {2} seconds".format(success,browser,round(time.time()-t1,2)))
        except Exception:
    ...
    def take_screenshot(self, file_path):
        ...
        try:
            ...
            self.my_print("{0} Get the current window screenshot,path: {1}, Spend {2} seconds".format(success,
                file_path, round(time.time() - t1,2)))
        except Exception:
    ...
    def open(self, url):
        ...
        try:
            self.driver.get(url)
            self.my_print("{0} Navigated to {1}, Spend {2} seconds".format(success,url,round(time.time()-t1,2)))
        except Exception:
            self.my_print("{0} Unable to load {1}, Spend {2} seconds".format(fail, url, round(time.time() -t1,2)))
            raise
    ...
    def max_window(self):
        ...
        self.my_print("{0} Set browser window maximized, Spend {1} seconds".format(success, round(time.time() - t1,2)))
    ...
    def set_window(self, wide, high):
        ...
        self.my_print("{0} Set browser window wide: {1},high: {2}, Spend {3} seconds".format(success,
            wide,high,round(time.time() - t1,2)))
    ...
    def type(self, css, text):
        ...
        try:
            ...
            self.driver.execute_script("arguments[0].style.border=\'2px dashed red\'", el)
            el.send_keys(text)
            self.my_print("{0} Typed element: <{1}> content: {2}, Spend {3} seconds".format(success,
                css,text,round(time.time() - t1,2)))
        except Exception:
            pngname = self.get_title()
            self.take_screenshot(Erro_img + now + '%s.png' % pngname)
    ...
    def click(self, css):
        ...
        try:
            ...
            self.driver.execute_script("arguments[0].style.border=\'2px dashed red\'", el)
            el.click()
            self.my_print("{0} Clicked element: <{1}>, Spend {2} seconds".format(success,css,round(time.time() - t1,2)))
        except Exception:
            pngname = self.get_title()
            self.take_screenshot(Erro_img + now +'%s.png'%pngname)
            self.my_print("{0} Unable to click element: <{1}>, Spend {2} seconds".format(fail, css, round(time.time() - t1,2)))
            raise
    ...
    def right_click(self, css):
        ...
        try:
            ...
            self.my_print("{0} Right click element: <{1}>, Spend {2} seconds".format(success, css, round(time.time() - t1,2)))
        except Exception:
            pngname = self.get_title()
            self.take_screenshot(Erro_img + now + '%s.png' % pngname)
            self.my_print("{0} Unable to right click element: <{1}>, Spend {2} seconds".format(fail, css, round(time.time() - t1,2)))
            raise
    ...
    def move_to_element(self, css):
        ...
        try:
            ...
            self.my_print("{0} Move to element: <{1}>, Spend {2} seconds".format(success, css, round(time.time() - t1,2)))
        except Exception:
            pngname = self.get_title()
            self.take_screenshot(Erro_img + now + '%s.png' % pngname)
            self.my_print("{0} unable move to element: <{1}>, Spend {2} seconds".format(fail, css, round(time.time() - t1,2)))
            raise
    ...
    def double_click(self, css):
        ...
        try:
            ...
            self.my_print("{0} Double click element: <{1}>, Spend {2} seconds".format(success, css, round(time.time() - t1,2)))
        except Exception:
            pngname = self.get_title()
            self.take_screenshot(Erro_img + now + '%s.png' % pngname)
            self.my_print("{0} Unable to double click element: <{1}>, Spend {2} seconds".format(fail, css, round(time.time() - t1,2)))
            raise
    ...
    def drag_and_drop(self, el_css, ta_css):
        ...
        try:
            ...
            self.my_print("{0} Drag and drop element: <{1}> to element: <{2}>, Spend {3} seconds".format(success,
                el_css,ta_css, round(time.time() - t1,2)))
        except Exception:
            pngname = self.get_title()
            self.take_screenshot(Erro_img + now + '%s.png' % pngname)
    ...
    def click_text(self, text):
        ...
        t1 = time.time()
        try:
            el = self.driver.find_element_by_partial_link_text(text)
            self.driver.execute_script("arguments[0].style.border=\'2px dashed red\'", el)
            el.click()
            self.my_print("{0} Click by text content: {1}, Spend {2} seconds".format(success, text,round(time.time() - t1,2)))
        except Exception:
            pngname = self.get_title()
            self.take_screenshot(Erro_img + now + '%s.png' % pngname)
            self.my_print("{0} Unable to Click by text content: {1}, Spend {2} seconds".format(fail, text, round(time.time() - t1,2)))
            raise
    ...
    def close(self):
        ...
        t1 = time.time()
        self.driver.close()
        self.my_print("{0} Closed current window, Spend {1} seconds".format(success, round(time.time() - t1,2)))
    ...
    def quit(self):
        ...
        t1 = time.time()
        self.driver.quit()
        self.my_print("{0} Closed all window and quit the driver, Spend {1} seconds".format(success, round(time.time() - t1,2)))
    ...
    def submit(self, css):
        ...
        try:
            ...
            self.driver.execute_script("arguments[0].style.border=\'2px dashed red\'", el)
            el.submit()
            self.my_print("{0} Submit form args element: <{1}>, Spend {2} seconds".format(success,css, round(time.time() - t1,2)))
        except Exception:
            self.my_print("{0} Unable to submit form args element: <{1}>, Spend {2} seconds".format(fail, css, round(time.time() - t1,2)))
            raise
    ...
    def F5(self):
        ...
        t1 = time
        self.driver.refresh()
        self.my_print("{0} Refresh the current page, Spend {1} seconds".format(success, round(time.time() - t1,2)))
    ...
    def js(self, script):
        ...
        try:
            ...
            self.my_print("{0} Execute javascript scripts: {1}, Spend {2} seconds".format(success,script, round(time.time() - t1,2)))
        except Exception:
            self.my_print("{0} Unable to execute javascript scripts: {1}, Spend {2} seconds".format(fail,
                script, round(time.time() - t1,2)))
            raise
    ...
    def get_attribute(self, css, attribute):
        ...
        try:
            ...
            self.my_print("{0} Get attribute element: <{1}>,attribute: {2}, Spend {3} seconds".format(success,
                css,attribute,round(time.time() - t1,2)))
            return attr
        except Exception:
    ...
    def get_text(self, css):
        ...
        try:
            ...
            self.my_print("{0} Get element text element: <{1}>, Spend {2} seconds".format(success,css,round(time.time() - t1,2)))
            return text
        except Exception:
            self.my_print("{0} Unable to get element text element: <{1}>, Spend {2} seconds".format(fail, css, round(time.time() - t1,2)))
            raise
    ...
    def get_title(self):
        ...
        self.my_print("{0} Get current window title, Spend {1} seconds".format(success, round(time.time() - t1,2)))
        return title
    ...
    def get_url(self):
        ...
        self.my_print("{0} Get current window url, Spend {1} seconds".format(success, round(time.time() - t1,2)))
        return url
    ...
    def accept_alert(self):
        ...
        self.my_print("{0} Accept warning box, Spend {1} seconds".format(success, round(time.time() - t1,2)))
    ...
    def dismiss_alert(self):
        ...
        self.my_print("{0} Dismisses the alert available, Spend {1} seconds".format(success, round(time.time() - t1,2)))
    ...
    def switch_to_frame(self, css):
        ...
        try:
            ...
            self.my_print("{0} Switch to frame element: <{1}>, Spend {2} seconds".format(success,css, round(time.time() - t1,2)))
        except Exception:
            pngname = self.get_title()
            self.take_screenshot(Erro_img + now + '%s.png' % pngname)
            self.my_print("{0} Unable switch to frame element: <{1}>, Spend {2} seconds".format(fail, css, round(time.time() - t1,2)))
            raise
    ...
    def switch_to_frame_out(self):
        ...
        self.my_print("{0} Switch to frame out, Spend {1} seconds".format(success, round(time.time() - t1,2)))
    ...
    def element_exist(self, css):
        ...
        try:
            self.element_wait(css)
            self.my_print("{0} Element: <{1}> is exist, Spend {2} seconds".format(success,css, round(time.time() - t1,2)))
            return True
        except TimeoutException:
            self.my_print("{0} Element: <{1}> is not exist, Spend {2} seconds".format(fail, css, round(time.time() - t1,2)))
            return False
    ...
    def into_new_window(self):
        ...
        try:
            ...
            self.my_print("{0} Switch to the new window,new window's url: {1}, Spend {2} seconds".format(success,
                self.driver.current_url,round(time.time() - t1,2)))
        except Exception:
            self.my_print("{0} Unable switch to the new window, Spend {1} seconds".format(fail, round(time.time() - t1,2)))
            raise
    ...
    def js_click(self, css):
        ...
        try:
            ...
            self.my_print("{0} Use javascript click element: {1}, Spend {2} seconds".format(success,js_str,round(time.time() - t1,2)))
        except Exception:
            self.my_print("{0} Unable to use javascript click element: {1}, Spend {2} seconds".format(fail,
                js_str, round(time.time() - t1,2)))
            raise
